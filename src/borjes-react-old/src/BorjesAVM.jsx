...
class BorjesAVM extends React.Component {
    ...
    render() {
        ...
        var file = this.props.filename;
        var showInput = this.state.showInput;
        var atrs = x.f;
        var refresh = this.props.refresh;
        var edit = this.state.edit;
        var showPrev2 = this.state.showPrev2;
/*        if (x.borjes === 'tfstruct' && atrs.length == 0) {
        ...
        if (edit) {
                ...
                <table className="borjes_fs">
                    ...
                    <tr>
                        <th colSpan="2">
                            <BorjesComponent x={x.type || Type} refresh={refresh} update={this.updateType.bind(this)}
                                             edit={edit} opts={opts}/>
                        </th>
                    </tr>
                    </thead> : null}
                   ...
                   <tr>
                        <td>
                            {edit ? <input ref={d => this.newFeature = d} type="text" size="40" id="newFeature"
                                           onKeyUp={this.newFeatureF.bind(this)}/> : null}
                        </td>
                    </tr>
                   {edit ? <tr>
                           <td><span style={{'cursor': 'pointer'}} onClick={this.turnEditOff.bind(this)}>-</span></td>
                       </tr>
                       : <tr>
                           <td><span style={{'cursor': 'pointer', 'fontSize': 'small'}}
